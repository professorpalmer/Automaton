...
Do not pass `--implement` against Automaton. Stop kills the `run` process Automaton spawned. After `job_id` prints, keep draining CLI stdout or the orchestrator blocks on a full pipe.

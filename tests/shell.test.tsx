...
describe('onSend scheduling', () => {
  ...
  test('Feed pin identity ignores last-text length and memos painted rows', () => {
    ...
    expect(src).toContain('MouthWaitBubble')
    expect(src).not.toContain('function ThinkingRow(')
    const wait = readFileSync(join(import.meta.dir, '../src/chrome/mouth-wait.tsx'), 'utf8')
    expect(wait).toContain('thinkingDots(3)')
    expect(wait).not.toContain('setInterval')
    expect(wait).not.toContain('setTimeout')
  })
})
...
describe('sister blob presentation', () => {
  ...
  test('selected lifts, idle breathes, and eyes live on every sister', () => {
    ...
    expect(open[0]?.lid).toBe(T.blob.eye)
    expect(shut[0]?.height).toBe(open[0]?.height)
    expect(shut[0]?.width).toBe(open[0]?.width)
    expect(shut[0]?.left).toBe(open[0]?.left)
    expect(shut[0]?.top).toBe(open[0]?.top)
    expect(shut[0]?.lid).toBe(T.space.xxs)
    expect(shut[0]?.lid).toBeLessThan(open[0]?.lid ?? 0)
    expect(Math.abs((glance[0]?.left ?? 0) - (open[0]?.left ?? 0))).toBeGreaterThan(0)
    expect(Number.isInteger(glance[0]?.left)).toBe(true)
    expect(Number.isInteger(glance[0]?.top)).toBe(true)
    const slitOpen = busyEyeLayout(0, false, 'slit')
    const slitShut = busyEyeLayout(0, true, 'slit')
    expect(slitOpen[0]?.width).toBe(T.blob.eye + 1)
    expect(slitOpen[0]?.height).toBe(Math.max(2, T.blob.eye - 1))
    expect(slitShut[0]?.width).toBe(slitOpen[0]?.width)
    expect(slitShut[0]?.top).toBe(slitOpen[0]?.top)
    expect(slitShut[0]?.left).toBe(slitOpen[0]?.left)
    expect(slitShut[0]?.lid).toBe(T.space.xxs)
    const cloud = busyEyeLayout(0, false, 'dot', null, 'cloud')
    expect(cloud[0]?.left).not.toBe(open[0]?.left)
    expect(EYE_ANCHOR.cloud).toEqual({ x: 19, y: 16 })
    expect(EYE_ANCHOR.wedge).toEqual({ x: 19, y: 17 })
    expect(EYE_ANCHOR.teardrop).toEqual({ x: 19, y: 18 })
    expect(EYE_ANCHOR.arch).toEqual({ x: 19, y: 15 })
    expect(EYE_ANCHOR.leaf).toEqual({ x: 19, y: 16 })
    expect(EYE_ANCHOR.crystal).toEqual({ x: 19, y: 16 })
    ...
    expect(blobSrc).toMatch(/import \{ GELATIN, motion \} from '@gpuix\/react'/)
    expect(blobSrc).not.toMatch(/const GELATIN =/)
    ...
    expect(blobSrc).toMatch(/stiffness: 13/)
    expect(blobSrc).toMatch(/damping: 14/)
    expect(blobSrc).toMatch(/mass: 1/)
    ...
    expect(blobSrc).toMatch(/SpringBox/)
    ...
    expect(blobSrc).toMatch(/<motion\.div/)
    expect(blobSrc).not.toMatch(/layoutDuration/)
    expect(blobSrc).toMatch(/transition=\{BODY_SPRING\}/)
    expect(blobSrc).toMatch(/transition=\{EYE_SPRING\}/)
  ...
  test('gelatin springs, five looks, and no idle ellipse squash', () => {
    ...
    const src = readFileSync(join(import.meta.dir, '../src/blob.tsx'), 'utf8')
    expect(src).toMatch(/import \{ GELATIN, motion \} from '@gpuix\/react'/)
    expect(src).toMatch(/<motion\.div/)
    expect(src).toMatch(/immediate=\{lifeImmediate\}/)
    expect(src).not.toMatch(/const GELATIN =/)
    ...
    expect(src).toMatch(/stiffness: 13/)
    expect(src).toMatch(/damping: 14/)
    expect(src).toMatch(/mass: 1/)
    expect(src).toMatch(/pointer\.down/)
    expect(src).not.toMatch(/delay: side \* 0\.05/)
    expect(src).not.toMatch(/style\.hover/)
    expect(src).not.toMatch(/hover: \{ opacity: T\.blob\.hover \}/)
    expect(src).toMatch(/setBlink\(true\)/)
    expect(src).toMatch(/lidOpen/)
    expect(src).toMatch(/SpringBox/)
    expect(src).toMatch(/blobClockShouldHold/)
    expect(src).not.toMatch(/layoutDuration/)
    expect(src).toMatch(/transition=\{BODY_SPRING\}/)
    expect(src).toMatch(/transition=\{EYE_SPRING\}/)
    ...
    expect(app).toMatch(/MouthWaitBubble/)
    expect(app).not.toMatch(/setStep\(\(n\) => n \+ 1\), T\.feed\.thinkMs/)
    const wait = readFileSync(join(import.meta.dir, '../src/chrome/mouth-wait.tsx'), 'utf8')
    expect(wait).toMatch(/thinkingDots\(3\)/)
    expect(wait).not.toMatch(/setStep\(\(n\) => n \+ 1\)/)
  })
...
native('staff shell (GPUI native)', () => {
  ...
  test(
    ...
    expect(findTestId(asTree(JSON.parse(renderer.getAutomationTree())), 'settings-compact')).toBeTruthy()
    expect(findTestId(asTree(JSON.parse(renderer.getAutomationTree())), 'settings-compact-now')).toBeTruthy()
  ...
  test('settings keeps sister seats collapsed and shows frost controls', () => {
    ...
    const pane = boundsFor(renderer, 'settings')
    expect(text, `more=${JSON.stringify(more)} pane=${JSON.stringify(pane)}`).toContain('Puppetmaster')

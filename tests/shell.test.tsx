...
describe('sister blob presentation', () => {
  ...
  test('each sister seeds its own clock from id, not a shared phase', () => {
    ...
    expect(lively.wanderMs).toBeGreaterThanOrEqual(T.blob.wanderMs * 1.15)
    expect(lively.wanderMs).toBeLessThanOrEqual(T.blob.wanderMs * 1.7)
  ...
  test('gelatin springs, five looks, and no idle ellipse squash', () => {
    ...
    expect(poseLayout('soft-wide')).toEqual({ left: -0.5, top: 0.5, width: 39, height: 37 })
    expect(poseLayout('soft-tall')).toEqual({ left: 0.5, top: -0.5, width: 37, height: 39 })

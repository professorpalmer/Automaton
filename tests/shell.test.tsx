...
import { assertSeedFrames, blobClock, blobDoubleBlink, blobNeedsClock, BUSY_LOOKS, busyEyeLayout, nextLook, presentBlob, shapeSvgSource, SisterBlob } from '../src/blob'
import {
  DEFAULT_AGENTS,
  emptyThreads,
...
describe('sister blob presentation', () => {
  ...
  test('selected lifts, idle breathes, and eyes live on every sister', () => {
    ...
    expect(stamped).toContain('width="76"')
    expect(stamped).toContain('height="76"')
    ...
    const wash = shapeSvgSource('blob', '#00C972', T.blob.size, 3)
    expect(wash).toContain('stroke-width')
    expect(wash).toContain('fill="#00C972"')
    expect(wash).toContain('overflow="visible"')
    ...
    expect(blobSrc).toMatch(/washHex\(fill\)/)
    ...
    expect(blobSrc).toMatch(/hover:\s*\{\s*opacity:\s*T\.blob\.hover/)
    ...
    expect(blobSrc).toMatch(/WASH_INFLATE_PX = 3/)
  ...
  test('gelatin springs, five looks, and no idle ellipse squash', () => {
    ...
    expect(src).toMatch(/hover: \{ opacity: T\.blob\.hover \}/)
    expect(src).toMatch(/setBlink\(true\)/)
    expect(src).toMatch(/height: eye\.height/)

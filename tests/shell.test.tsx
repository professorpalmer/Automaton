...
native('staff shell (GPUI native)', () => {
  ...
  test('goal blocker sits on the dock, not the transcript; Retry and Cancel work', () => {
    ...
    const retryRoot = createTestRoot()
    retryRoot.render(<App store={retryStore} />)
    retryRoot.renderer.flush()
    const shot = 'artifacts/shots/shell-goal-blocker.png'
    retryRoot.renderer.captureScreenshot(shot)
    let tree = asTree(JSON.parse(retryRoot.renderer.getAutomationTree()))
    ...
    const painted = retryRoot.renderer.getPaintedText().join(' ')
    ...
    expect(statSync(shot).size).toBeGreaterThan(1000)
    clickTestId(retryRoot.renderer, 'goal-blocker-retry')
    retryRoot.renderer.flush()
    tree = asTree(JSON.parse(retryRoot.renderer.getAutomationTree()))
